...
/**
 * A factory class for converting a user to one of his subclasses.
 * The class checks that the user is of an appropriate role before returning
...
class UserFactory {
	...
	static SocialWorker createSocialWorker(User user) throws IncompatiblePrivilegesException {
		if (Role.SOCIAL_WORKER.equals(user.getRole()) ||
				Role.SYSTEM_ADMIN.equals(user.getRole())) {
	...
	static Donor createDonor(User user) throws IncompatiblePrivilegesException {
		if (Role.DONOR.equals(user.getRole()) ||
				Role.SYSTEM_ADMIN.equals(user.getRole())) {
	...
	static Receiver createReceiver(User user) throws IncompatiblePrivilegesException {
		if (Role.RECEIVER.equals(user.getRole()) ||
				Role.SYSTEM_ADMIN.equals(user.getRole())) {
